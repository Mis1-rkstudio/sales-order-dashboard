...
export async function GET(request: Request) {
  ...
  if (!projectId || !dataset) {
    return NextResponse.json({ error: 'Missing BQ_PROJECT or BQ_DATASET env vars' }, { status: 500 });
  }
  ...
  const queryParams: Record<string, string | number> = { limit: limitNum, offset: offsetNum };

  if (params.q) {
    filters.push(`(LOWER(Product_Code) LIKE LOWER(@q) OR LOWER(Concept_2) LIKE LOWER(@q) OR LOWER(Concept_3) LIKE LOWER(@q))`);
  ...
  try {
    ...
    const rows = rowsResult as { Concept?: string; Fabric?: string; Item?: string; File_URL?: string }[];
    ...
    const total =
      Array.isArray(countResult) && countResult.length > 0 ? Number((countResult[0] as { cnt?: string | number }).cnt ?? rows.length) : rows.length;
